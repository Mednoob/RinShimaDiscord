...
class MusicManager {
    ...
    async handleVideo(message, voiceChannel, song) {
        ...
        if (!serverQueue) {
            ...
            try {
                ...
                message.channel.send({embed: {color: "RED", description: `I could not join the voice channel: ${error.message}`}});
            }
        } else {
            serverQueue.songs.push(song);
            message.channel.send({embed: {color: "GREEN", description: `Successfully added **${song.info.title}** to the queue!`}});
        }
    }
    ...
    play(guild, song) {
        ...
        if (!song) {
            serverQueue.textChannel.send({embed: {color:"BLUE", description: "Queue is empty! Leaving voice channel.."}});
            ...
            serverQueue.textChannel.send({embed: {color: "BLUE", description: `Now playing: **${song.info.title}** by *${song.info.author}*`}});
        }
    }
