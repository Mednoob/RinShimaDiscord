...
export default class BlurCommand extends BaseCommand {
    ...
    public async execute(ctx: CommandQueryContext): Promise<void> {
        const url = (ctx.isMessage() ? ctx.context.attachments.first()?.url : undefined) ?? (this.rin.utils.isUrl(ctx.args[0]) ? ctx.args[0] : undefined) ?? (this.rin.users.cache.get(ctx.args[0]?.replace(/[^0-9]/g, "")) ?? (ctx.isMessage() ? ctx.context.member! : (ctx.isInteraction() ? ctx.context.user : undefined)))?.displayAvatarURL({ size: 2048, format: "png" });
        ...
        try {
            ...
            void ctx.reply({
                files: [
                    new MessageAttachment(edited, "blur.png")
                ]
            });
        } catch {
            void ctx.reply({
